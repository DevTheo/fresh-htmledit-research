// Borrowed from: https://github.com/rob-race/react-trix/tree/main
...
export interface TrixEditorProps {
  ...
  mergeTags: Array<MergeTags> | null;
...
export class TrixEditor extends Component<TrixEditorProps, TrixEditorState> {
  private id: string;
  ...
  private uploadAttachment(attachment: any) {
    ...
    xhr.open("POST", (this as any).props.uploadURL, true);
  ...
  private handleTagSelected(t: MergeTag, e: MouseEvent): void {
    e.preventDefault();
  ...
  private renderTagSelector(tags: Array<MergeTag>): any {
    if (!tags || !this.editor) {
      return null;
    }
    ...
    const boxStyle = {
      ...
      "top": rect.top + 25 - (editorPosition?.top || 0),
      "left": rect.left + 25 - (editorPosition?.left || 0),
      "width": "250px",
  ...
  render() {
    ...
    let mergetags: any /*React.ReactNode*/ = null;
    if (state.showMergeTags) {
      mergetags = this.renderTagSelector(state.tags);
    }
    return (
      <div id={"trix-editor-top-level-" + this.id} ref={(d) => (this as any).d = d} style={{ "position": "relative" }}>
        {createElement("trix-editor", attributes)}
        <input
          type="hidden"
          id={`input-${this.id}`}
